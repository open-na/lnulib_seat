        ...
        if sit_avilable is None:
            ...
            if sit_avilable is None:
                ...
                if sit_avilable is None:
                    ...
                    sys.exit()
import hashlib
def get_monthly_hash():
    ym = time.strftime("%Y%m")  # 例如 "202506"
    input_str = f"{ym}"
    return hashlib.sha256(input_str.encode()).hexdigest()[:10]  # 取前10位作为后缀
    ...
    for idx, (account, password) in enumerate(total_accounts):
        ...
        if CDKEY == monthly_hash:
            ...
            try:
                ...
                profile_dir = tempfile.mkdtemp(prefix=f"edge_profile_{account}_{monthly_hash}_{str(idx)}")
